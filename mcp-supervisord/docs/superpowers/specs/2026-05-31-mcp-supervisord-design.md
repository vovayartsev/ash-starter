# mcp-supervisord — Design Spec

**Date:** 2026-05-31
**Status:** Draft / awaiting user review
**Scope:** New Python service `mcp-supervisord` providing a unified MCP HTTP endpoint that (a) proxies one or more stdio MCP servers and (b) manages local processes with structured log capture. Runs inside the project's devcontainer.
...
`tools.json` schema is documented in `mcp-supervisord/README.md` (deliverable).
...
├── README.md               # tools.json schema reference + usage
├── src/mcp_supervisord/
│   ├── __init__.py
...
`mcp-supervisord` is installed and started by the devcontainer:
...
CI target: `uv run pytest -q` from `mcp-supervisord/`.

## 13. Deliverables

1. `tools.json` at repo root (✅ already written).
2. `mcp-supervisord/` package per §10.
3. `mcp-supervisord/README.md` documenting full `tools.json` schema, supervisor tools (§6), `status` shape (§7), Claude Desktop wiring snippet.
