...
While iterating, run straight from the package directory:

```sh
uv venv -p 3.11 .venv
uv pip install -e .
.venv/bin/mcp-supervisord --config /path/to/tools.json
```

After publishing (git repo, PyPI, …) install once and run as a CLI tool:

```sh
uv tool install --from git+https://github.com/<you>/mcp-supervisord mcp-supervisord
# or, from a checkout:
uv tool install --editable ./mcp-supervisord
mcp-supervisord --config /workspaces/flylixir/tools.json
```

`uvx` works the same way: `uvx --from git+https://… mcp-supervisord -c tools.json`.

CLI flags:

| Flag | Meaning |
|---|---|
| `-c, --config PATH` | path to `tools.json` (required) |
